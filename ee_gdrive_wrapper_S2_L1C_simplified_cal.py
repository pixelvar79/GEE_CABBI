...
def toa_to_sr(image):
    """Converts TOA to SR using a simple Dark Object Subtraction (DOS1) method."""
    opticalBands = ['B2', 'B3', 'B4', 'B5', 'B8']
    toa = image.select(opticalBands)
    
    # Reduce region to get dark object values
    darkObject = toa.reduceRegion(
    ...
    # Convert darkObject values to ee.List and then to ee.Image
    darkObjectList = ee.List(darkObject.values())
    darkObjectImage = ee.Image.constant(darkObjectList)
    
    # Subtract dark object values from TOA reflectance
    sr = toa.subtract(darkObjectImage)
    
    # Rename the bands to indicate surface reflectance
    sr = sr.rename([b + '_SR' for b in opticalBands])
    
    # Add the surface reflectance bands to the original image
...
def export_s2(bounding_box, start_date, end_date, shp_name):
    ...
    s2_sr_cld_col_with_clouds_masks_cal = s2_sr_cld_col_with_clouds_masks.map(toa_to_sr)
    

    filtered_count = s2_sr_cld_col_with_clouds_masks_cal.size().getInfo()
    ...
    if filtered_count > 0:
        ...
        captured_dates = s2_sr_cld_col_with_clouds_masks_cal.aggregate_array('system:time_start').getInfo()
        ...
        images = s2_sr_cld_col_with_clouds_masks_cal.toList(filtered_count)
